...
public:
    ...
    void printout() const
    {
        ...
        for(int i = 0; i < n_cmds; i++)
            HOTSTUFF_LOG_INFO("In the orderedlist, (cmd,timestamp): (%s,%s)",get_hex10(cmds[i]).c_str(),boost::lexical_cast<std::string>(timestamps[i]).c_str());
    }
...
public:
    ...
    std::vector<uint64_t> get_timestamps_1(const LeaderProposedOrderedList &proposed_orderedlist) const;
