...
class HotStuffCore {
    ...
    int round_number; 
    int passed_round;
    std::vector< Aequitas::TopologyGraph<max_num_all_txn> > allG; /* TODO: change to deque? - topology graph of each round for order-fairness */
    std::map<uint256_t, hotstuff::cmd_with_type> map_cmd; /* a map for all the cmd */
    std::vector<uint256_t> cmd_content;
    int num_of_all_cmds;

    std::mutex command_insertion;
    BoxObj<EntityStorage> storage;
    BoxObj<CommandTimestampStorage> command_timestamp_storage;
    BoxObj<OrderedListStorage> orderedlist_storage;

    HotStuffCore(ReplicaID id, privkey_bt &&priv_key); //{round_number = 0;}
...
struct Vote : public Serializable
{
    ...
    //void preferredOrderedListPrintout() const {
    //	replica_preferred_orderedlist -> printout();
    //}
