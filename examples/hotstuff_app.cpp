...
int main(int argc, char **argv) {
    ...
    //Config config("scripts/deploy/hotstuff.gen.conf");	
...
HotStuffApp::HotStuffApp(uint32_t blk_size,
    ...
    // HOTSTUFF_LOG_INFO("prepare to run cn.reg_handler(salticidae::generic_bind())");
    ...
    // HOTSTUFF_LOG_INFO("finish cn.listen(clisten_addr)");
}
...
void HotStuffApp::client_request_cmd_handler(MsgReqCmd &&msg, const conn_t &conn) {
    ...
    {
        HOTSTUFF_LOG_INFO("Adding command %s to storage", get_hex10(cmd->get_hash()).c_str());
...
void HotStuffApp::start(const std::vector<std::tuple<NetAddr, bytearray_t, bytearray_t>> &reps) {
    // HOTSTUFF_LOG_INFO("HotStuffApp started!");
    ...
    HotStuff::start(reps);
    // HOTSTUFF_LOG_INFO("Successfully came out from Hotstuff::start(reps).");
    cn.reg_conn_handler([this](const salticidae::ConnPool::conn_t &_conn, bool connected) {
	// HOTSTUFF_LOG_INFO("Inside client network's reg_con_handler!!");
        auto conn = salticidae::static_pointer_cast<conn_t::type>(_conn);
        if (connected){
            client_conns.insert(conn);
            #ifdef HOTSTUFF_ENABLE_BENCHMARK
            HOTSTUFF_LOG_INFO("Connected with the client: %s", std::string(*conn).c_str());
            #endif
	}else{
            client_conns.erase(conn);
            #ifdef HOTSTUFF_ENABLE_BENCHMARK
	        HOTSTUFF_LOG_INFO("Not connected with the client: %s", std::string(*conn).c_str());
            #endif
	}
	return true;
    });
    // HOTSTUFF_LOG_INFO("Successfully finish cn.reg_conn_handler().");
    req_thread = std::thread([this]() { req_ec.dispatch(); });
    resp_thread = std::thread([this]() { resp_ec.dispatch(); });
    /* enter the event main loop */
    ec.dispatch();
    // HOTSTUFF_LOG_INFO("ec.dispatch() done!!");
}
