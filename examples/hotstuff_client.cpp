...
void connect_all()
{
    ...
    {
	// HOTSTUFF_LOG_INFO("replicas[%d]=%d", i, replicas[i]);
        conns.insert(std::make_pair(i, mn.connect_sync(replicas[i])));
        // HOTSTUFF_LOG_INFO("End of connect all for replica %d",i);
    }
}
...
bool try_send(bool check = true)
{
    ...
    if (!check || waiting.size() < max_async_num)
    {
        ...
        for (auto &p : conns)
	{
		// HOTSTUFF_LOG_INFO("p.second = %d", p.second);
...
Config config("hotstuff.conf");
    ...
    for (const auto &s : opt_replicas->get())
    {
        ...
        raw.push_back(res[0]);
	// HOTSTUFF_LOG_INFO("res[0] = %d is being pushed into raw.", res[0]);
    }
